...
// Allocate an AWDB_data_pool_s. It initially has space for size
// AWDB_entry_data_list_s structs.
AWDB_data_pool_s *data_pool_new(size_t const size)
{
    AWDB_data_pool_s *const pool = calloc(1, sizeof(AWDB_data_pool_s));
    if (!pool) {
        return NULL;
    }

    if (size == 0 ||
        !can_multiply(SIZE_MAX, size, sizeof(AWDB_entry_data_list_s))) {
        data_pool_destroy(pool);
        return NULL;
    }
    pool->size = size;
    pool->blocks[0] = calloc(pool->size, sizeof(AWDB_entry_data_list_s));
    if (!pool->blocks[0]) {
        data_pool_destroy(pool);
        return NULL;
    }
...
void data_pool_destroy(AWDB_data_pool_s *const pool)
{
    if (!pool) {
        return;
    }
...
// grow.
AWDB_entry_data_list_s *data_pool_alloc(AWDB_data_pool_s *const pool)
{
    if (!pool) {
        return NULL;
    }
    ...
    if (pool->used < pool->size) {
        AWDB_entry_data_list_s *const element = pool->block + pool->used;
        pool->used++;
        return element;
    }
    ...
    if (!can_multiply(SIZE_MAX, new_size, sizeof(AWDB_entry_data_list_s))) {
        return NULL;
    }
    pool->blocks[new_index] = calloc(new_size, sizeof(AWDB_entry_data_list_s));
    ...
    AWDB_entry_data_list_s *const element = pool->block;
    pool->used = 1;
    return element;
}
...
AWDB_entry_data_list_s *data_pool_to_list(AWDB_data_pool_s *const pool)
{
    if (!pool) {
        return NULL;
    }
    ...
    for (size_t i = 0; i <= pool->index; i++) {
        AWDB_entry_data_list_s *const block = pool->blocks[i];
        ...
        for (size_t j = 0; j < size - 1; j++) {
            AWDB_entry_data_list_s *const cur = block + j;
            cur->next = block + j + 1;
        }

        if (i < pool->index) {
            AWDB_entry_data_list_s *const last = block + size - 1;
...
static void test_can_multiply(void)
{
    ...
    {
        ok(can_multiply(SIZE_MAX, 10240, sizeof(AWDB_entry_data_list_s)),
